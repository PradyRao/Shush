...
async def user_join(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    if str(after.channel.id) not in var_config.appliedchs[str(member.guild.id)]:
        # we don't mute them
...
async def user_leave(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    if str(before.channel.id) in var_config.appliedchs[str(member.guild.id)]:
...
async def user_move(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    if (before.channel.id != after.channel.id) \
            and (str(before.channel.id) in var_config.appliedchs[str(member.guild.id)]) \
            and (str(after.channel.id) in var_config.appliedchs[str(member.guild.id)]):
    ...
    elif str(before.channel.id) not in var_config.appliedchs[str(member.guild.id)] and str(after.channel.id) in var_config.appliedchs[str(member.guild.id)]:
        await member.edit(mute=True)
    ...
    elif str(before.channel.id) in var_config.appliedchs[str(member.guild.id)] and str(after.channel.id) not in var_config.appliedchs[str(member.guild.id)]:
        await member.edit(mute=False)
        await process_practice.process_leave_end(member, before)
