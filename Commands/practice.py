...
async def practice(ctx: discord.ext.commands.Context):
    ...
    # check if someone is already practicing
    elif str(ctx.author.voice.channel.id) in var_config.practicemap[str(ctx.guild.id)].keys():
        ...
        var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id)] = str(ctx.author.id)
...
async def room_status(ctx: discord.ext.commands.Context):
    ...
    # check to see if there is a user currently practicing in the voice channel
        ...
        time_practiced_seconds = time_utils.time_practiced_seconds(ctx.guild.id, ctx.author.voice.channel.id)
        ...
        if str(ctx.author.voice.channel.id) + 'piece' in var_config.practicemap[str(ctx.guild.id)].keys():
            await ctx.reply(f'{practicing_user.name}#{practicing_user.discriminator} has been practicing for {time_practiced[1]} hours {time_practiced[2]} minutes {time_practiced[3]} seconds. '
                            f'User is currently practicing: {var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id) + "piece"]}')
        else:
            await ctx.reply(f'{practicing_user.name}#{practicing_user.discriminator} has been practicing for {time_practiced[1]} hours {time_practiced[2]} minutes {time_practiced[3]} seconds.')
    return
...
async def practice_piece(ctx: discord.ext.commands.Context, piece):
    ...
    # check if a user is practicing in the voice channel and if the user that executed the command is currently the one practicing in this voice channel
    elif str(ctx.author.voice.channel.id) not in var_config.practicemap[str(ctx.guild.id)].keys() or var_config.practicemap[str(
            ctx.guild.id)][str(ctx.author.voice.channel.id)] != str(ctx.author.id):
        await ctx.reply('you are not practicing in this voice channel')
    elif not piece:
        ...
        logging.log(level=logging.INFO,
                    msg=f'practicing user {ctx.author.name}#{ctx.author.discriminator} id: {ctx.author.id} has set their piece to {piece} in channel {ctx.author.voice.channel.name} id:'
                        f' {ctx.author.voice.channel.id}')
        var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id) + 'piece'] = str(piece)
