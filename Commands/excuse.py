    ...
    # check if a user is practicing in the voice channel and if the user that executed the command is currently the one practicing in this voice channel, or a moderator
        ...
        for member in args:
            ...
            if (member.id != ctx.author.id) and (member in ctx.author.voice.channel.members):
                ...
                if str(ctx.author.voice.channel) + 'excused' not in var_config.practicemap[str(ctx.guild.id)].keys():
                    var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id) + 'excused'] = [member.id]
                else:
                    var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id) + 'excused'].append(member.id)
                # un-mute them
    ...
    # check if a user is practicing in the voice channel and if the user that executed the command is currently the one practicing in this voice channel, or a moderator
        ...
        for member in args:
            ...
            if (member.id != ctx.author.id) \
                ...
                var_config.practicemap[str(ctx.guild.id)][str(ctx.author.voice.channel.id) + 'excused'].remove(member.id)
                # re-mute them
